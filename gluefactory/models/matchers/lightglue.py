...
class LightGlue(nn.Module):
    ...
    """
    def loss(self, pred, data, use_gt=False):
        
        #Fonction de perte améliorée avec plusieurs composantes pour l'apprentissage auto-supervisé.
        
        def loss_params(pred, i):
            la, sim = self.log_assignment[i](
        ...
        def consistency_loss(matches0, matches1, scores0, scores1):
            #Perte de consistance bidirectionnelle
            ...
            forward_consistency = (forward_matches == torch.arange(matches0.size(1), 
                                device=matches0.device)[None]).float()
            backward_consistency = (backward_matches == torch.arange(matches1.size(1), 
                                device=matches1.device)[None]).float()
            
            consistency = (forward_consistency * scores0 + backward_consistency * scores1) / 2
            return -consistency.mean()

        def descriptor_diversity_loss(desc0, desc1):
            #Encourage la diversité des descripteurs
            # Normaliser les descripteurs
            desc0 = F.normalize(desc0, p=2, dim=-1)
            desc1 = F.normalize(desc1, p=2, dim=-1)
            
            # Calculer les similarités intra-descripteurs
            sim0 = torch.matmul(desc0, desc0.transpose(-2, -1))
            sim1 = torch.matmul(desc1, desc1.transpose(-2, -1))
            
            # Masquer la diagonale
            mask = ~torch.eye(sim0.shape[-1], dtype=torch.bool, device=sim0.device)
            
            # Pénaliser les descripteurs trop similaires
            div_loss0 = torch.where(mask, F.relu(sim0 - 0.1), torch.zeros_like(sim0))
            div_loss1 = torch.where(mask, F.relu(sim1 - 0.1), torch.zeros_like(sim1))
            
            return (div_loss0.mean() + div_loss1.mean()) / 2

        # Initialiser les pertes
        losses = {}
        sum_weights = 1.0

        # Obtenir les paramètres de la dernière couche
        params_final = loss_params(pred, -1)
        
        # Perte de base (log-softmax)
        base_loss = -params_final["log_assignment"][:, :-1, :-1].mean()
        ...
        div_loss = descriptor_diversity_loss(
            pred["ref_descriptors0"][:, -1],
            pred["ref_descriptors1"][:, -1]
        )

        # Combiner les pertes
        losses["total"] = base_loss + 0.1 * cons_loss + 0.01 * div_loss

        # Pertes intermédiaires pour chaque couche
        ...
        if self.training:
            ...
            for i in range(N - 1):
                ...
                # Perte de correspondance pour cette couche
                layer_loss = -params_i["log_assignment"][:, :-1, :-1].mean()
                
                # Pondération
                weight = self.conf.loss.gamma ** (N - i - 1) if self.conf.loss.gamma > 0.0 else (i + 1)
                sum_weights += weight
                losses["total"] += layer_loss * weight
        ...
        # Ajouter la perte de confiance pendant l'entraînement
        if self.training:
            confidence_loss = losses["confidence"].mean() if isinstance(losses["confidence"], torch.Tensor) else losses["confidence"]
            losses["total"] += 0.1 * confidence_loss  # Réduire le poids de la perte de confiance
    ...
    def loss(self, pred, data):
        ...
        # S'assurer que nll est un scalaire
        nll = nll.mean()  # Ajouter cette ligne
        ...
        losses["row_norm"] = pred["log_assignment"].exp()[:, :-1].sum(2).mean()  # Modifier cette ligne

        for i in range(N - 1):
            params_i = loss_params(pred, i)
            nll_i, _, _ = self.loss_fn(params_i, data, weights=gt_weights)
        
            # S'assurer que nll_i est un scalaire
            nll_i = nll_i.mean()  # Ajouter cette ligne
            ...
            confidence_loss = self.token_confidence[i].loss(
                pred["ref_descriptors0"][:, i],
                pred["ref_descriptors1"][:, i],
                params_i["log_assignment"],
                pred["log_assignment"],
            ...
            # S'assurer que confidence_loss est un scalaire
            confidence_loss = confidence_loss.mean()  # Ajouter cette ligne
            losses["confidence"] += confidence_loss / (N - 1)
